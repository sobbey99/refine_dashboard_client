...
const AllProperties = () => {
 ...
 if (isError) {
  ...
  return (
    <Box>
      {/*  Sort Price, Search by title, Select Filter */}
      <Box mt="20px" sx={{
        display: "flex",
        flexWrap: "wrap",
        gap: 3
        ...
        <Typography fontSize={25} fontWeight={700} color="#11142d">
          ...
          <Box mb={2} mt={3} display="flex" width="84%" justifyContent="space-between" flexWrap="wrap">
            <Box display="flex" gap={2} flexWrap="wrap" mb={{
              xs: "20px",
              sm: 0
            }}>
              <CustomButton 
              ...
              >
                ...
                {["Apartment", "Villa", "Farmhouse", "Condos", "Townhouse", "Duplex", "Studio", "Chalet"].map((type) => {
                  const lowerCaseValue = type.toLowerCase()
                  return (
                    <MenuItem value={lowerCaseValue} key={type}>{type}</MenuItem>
                  )
                })}
              </Select>
            </Box>
